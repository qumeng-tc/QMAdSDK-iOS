...
@property (nonatomic, strong) NSMutableArray<id<QMBaseConf>> *confs;
...
- (void)viewDidLoad {
    ...
    QMAdSDKConfiguration *config = [QMAdSDKConfiguration shareConfiguration];
    NSString *personalAdIsEnable = [config isEnablePersonalAds] ? @"YES" : @"NO";
    NSString *versionInfo = [NSString stringWithFormat:@"个性化推荐: %@ \nversion: %@\nshort version: %@", personalAdIsEnable, [QMAdSDKManager sdkVersion], [QMAdSDKManager shortSdkVersion]];
        ...
        @{ @"title": @"广告: 信息流",
           @"list": @[
               ...
               @{
                   @"action": @"大图",
                   @"slot": @"9014850",
                   @"controller": @"QMFeedDemoViewController",
               ...
               @{
                   @"action": @"图片非列表",
                   @"slot": @"9014850",
                   @"controller": @"QMFeedSingleViewController",
                   @"SEL": NSStringFromSelector(@selector(performCustomJump:))
               },
               @{
                   @"action": @"视频非列表",
        ...
        @{ @"title": @"广告: 自渲染",
           @"list": @[
               ...
               @{
                   @"action": @"大图",
                   @"slot": @"9014850",
                   @"controller": @"QMNativeDemoViewController",
                   @"SEL": NSStringFromSelector(@selector(performCustomJump:))
               },
               @{
                   @"action": @"小图",
               ...
               @{
                   @"action": @"图片非列表",
                   @"slot": @"9014850",
                   @"controller": @"QMNativeSingleViewController",
                   @"SEL": NSStringFromSelector(@selector(performCustomJump:))
               },
               @{
                   @"action": @"视频非列表",
...
- (void)performHorizontalRotation:(NSDictionary *)params {
    [self qm_rotateToInterfaceOrientation:UIInterfaceOrientationLandscapeRight];
    currentVCInterfaceOrientationMask = UIInterfaceOrientationMaskLandscape;
    [self qm_setNeedsUpdateOfSupportedInterfaceOrientations];
}

- (void)performVerticalRotation:(NSDictionary *)params {
    currentVCInterfaceOrientationMask = UIInterfaceOrientationMaskPortrait;
    [self qm_rotateToInterfaceOrientation:UIInterfaceOrientationPortrait];
    [self qm_setNeedsUpdateOfSupportedInterfaceOrientations];
}
...
- (void)performSlotJump:(NSDictionary *)params {
    ...
    if ([className isEqualToString:@"QMRewardedVideoConf"]) {
        QMRewardedVideoConf *conf = (QMRewardedVideoConf *)_conf;
...
- (void)performCustomJump:(NSDictionary *)params {
    ...
    if ([className isEqualToString:@"QMFeedDemoViewController"]) {
        [(QMFeedDemoViewController *)controller setFeedcCustomSize: self.feedSize];
    }
...
- (void)customRetentionInfo:(NSDictionary *)params {
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        ...
        strongSelf.alertInfo = [[QMRetentionAlertInfo alloc] init];
