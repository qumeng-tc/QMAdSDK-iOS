...
@interface QMSplashAdConf ()<QMSplashAdDelegate>
...
- (void)qm_splashAdRenderFail:(QMSplashAd *)splashAd error:(NSError *)error {
    QMLog(@"【媒体】开屏: 物料渲染失败 Error: %@", error);
}
...
- (void)qm_splashAdShowFail:(QMSplashAd *)splashAd error:(NSError *)error {
    self.splashAd = nil;
    QMLog(@"【媒体】开屏: show失败 error：%@", error);
    if (self.delegate && [self.delegate respondsToSelector:@selector(splashAdDidClose)]) {
        [self.delegate splashAdDidClose];
    }
}
...
//            QMLog(@"倒计时: %ld 秒", (long)remainingTime);
//            remainingTime -= 1;
//        } else {
//            QMLog(@"倒计时结束！");
